...
function Usercourseslist() {
  ...
  const [filteredCourseDetails, setFilteredCourseDetails] = useState<Course[]>([]);//search
...
//to get wishlistCount
 ...
 // fetching and updating the entire course list
  useEffect(() => {
    if (searchedCourse) {
  ...
  // filtering the course list based on the search query entered by the user(SEARCH)
  useEffect(() => {
    if (searchQuery) {
  ...
  axiosInstance.get('/filtercourse', { params: { category: selectedCategory } })
    .then((response) => {
      if (response.data && response.data.filteredCourses) {
        console.log(response.data.filteredCourses, "filtered course");
        setFilteredCourseDetails(response.data.filteredCourses); // Update state with filtered courses
      }
    })
    .catch((error) => {
      console.log(error);
        ...
        <div className="min-h-screen">
          <h2 className="text-3xl font-bold mb-4">What to learn next</h2>       
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4" style={{ maxWidth: '1200px', margin: '0 auto' }}>            
            {filteredCourseDetails.map((course) => (
