...
import CourseIntro from "../../User/CourseDetail/CourseIntro";
    ...
    axiosInstanceAdmin
      ...
      .then((response) => {
        if (response.data) {
            ...
            const { notifications, notificationCount } = response.data;
          ...
          <div>
            ...
            >
              <div className="relative m-2 inline-flex">
                 <div className="absolute bottom-auto left-0 right-auto top-0 z-10 inline-block -translate-x-2/4 -translate-y-1/2 rotate-0 skew-x-0 skew-y-0 scale-x-100 scale-y-90 rounded-full bg-red-600 text-white p-1.5 text-xs">{notificationCount}</div> 
  ...
  {notifications.map((notification) => (
    ...
    >
      <div
        ...
        role="alert"
      >
        <h2 className="text-base font-semibold">{notification.type}</h2>
